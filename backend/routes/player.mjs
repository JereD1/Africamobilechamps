...
router.get('/players', async (req, res) => {
    ...
    try {
        ...
        if (filter && value) {
            players = await Player.find({ [filter]: { $regex: value, $options: 'i' } }); // Case insensitive search
        } else {
...
router.put('/players/:UID', async (req, res) => {
    ...
    try {
        const updatedPlayer = await Player.findOneAndUpdate({ UID }, req.body, { new: true });
...
router.patch('/players/:UID', async (req, res) => {
    ...
    try {
        const updatedPlayer = await Player.findOneAndUpdate({ UID }, { $set: req.body }, { new: true });
        if (!updatedPlayer) return res.sendStatus(404);
        return res.json(updatedPlayer);
    } catch (error) {
        console.error(error);
